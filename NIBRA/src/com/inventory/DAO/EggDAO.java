...
public class EggDAO {
    ...
    public void addEggDAO(EggDTO productDTO) {
        /*
        String query = "SELECT * FROM egg WHERE productname='"
        ...
        resultSet = statement.executeQuery(query);
        if (resultSet.next()) {
        JOptionPane.showMessageDialog(null, "Um novo produto foi adcionado.");
        } else {*/
        addFunction(productDTO);
        //  }
    }

    public void addFunction(EggDTO productDTO) {
    try {
        // Verifica e atualiza o estoque antes de inserir os dados do produto
            // Insere dados na tabela 'egg'
            String query = "INSERT INTO egg VALUES(null,?,?,?,?,?,?,?)";
            prepStatement = (PreparedStatement) conn.prepareStatement(query);
            prepStatement.setString(1, productDTO.getProdCode());
            prepStatement.setString(2, productDTO.getBrand());
            prepStatement.setString(3, productDTO.getProdType());
            prepStatement.setString(4, productDTO.getDate());
            prepStatement.setDouble(5, productDTO.getCostPrice());
            prepStatement.setDouble(6, productDTO.getSellPrice());
            prepStatement.setString(7, productDTO.getProdName());
            prepStatement.executeUpdate();

            // Insere dados na tabela 'products'
            String query3 = "INSERT INTO products VALUES(null,?,?,?,?,?,?)";
            prepStatement3 = (PreparedStatement) conn.prepareStatement(query3);
            prepStatement3.setString(1, productDTO.getProdCode());
            prepStatement3.setString(2, productDTO.getProdName());
            prepStatement3.setDouble(3, productDTO.getCostPrice());
            prepStatement3.setDouble(4, productDTO.getSellPrice());
            prepStatement3.setString(5, productDTO.getBrand());
            prepStatement3.setString(6, productDTO.getProdType());
            prepStatement3.executeUpdate();

            // Insere dados na tabela 'currentstock'
            ...
            prepStatement2.setString(3, productDTO.getProdType());
            prepStatement2.executeUpdate();

            JOptionPane.showMessageDialog(null, "Ovos adicionados e estão prontos para a venda.");
    ...
    public void addPurchaseDAO(ProductDTO productDTO) {
        try {
            ...
            prepStatement.executeUpdate();
            JOptionPane.showMessageDialog(null, "Registro de compra adcionado.");
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public void editProdDAO(EggDTO productDTO) {
        try {
            String query = "UPDATE egg SET productname=?,costprice=?,sellprice=?,brand=? WHERE productcode=?";
            prepStatement = (PreparedStatement) conn.prepareStatement(query);
            ...
            String query3 = "UPDATE products SET productname=?,costprice=?,sellprice=?,brand=? WHERE productcode=?";
            prepStatement = (PreparedStatement) conn.prepareStatement(query);
            prepStatement.setString(1, productDTO.getProdName());
            prepStatement.setDouble(2, productDTO.getCostPrice());
            prepStatement.setDouble(3, productDTO.getSellPrice());
            ...
            prepStatement.executeUpdate();
            prepStatement2.executeUpdate();
            JOptionPane.showMessageDialog(null, "Os detalhes do produto foram actualizados.");
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    }
    ...
    public void deleteSaleDAO(String ID) {
        try {
            String query = "DELETE FROM egg WHERE productcode=?";
            prepStatement = conn.prepareStatement(query);
            prepStatement.setString(1, ID);
            prepStatement.executeUpdate();

            JOptionPane.showMessageDialog(null, "Produto completamente removido");
        } catch (SQLException e) {
            e.printStackTrace();
        }
        deleteStock();
    ...
    public void sellProductDAO(ProductDTO productDTO, String username) {
        ...
        try {
            ...
            if (productDTO.getQuantity() > quantity) {
                JOptionPane.showMessageDialog(null, "Sem estoque suficiente para esse produto.");
    ...
    public ResultSet getEggsInfo() {
     try {
            String query = " SELECT productname,productcode,prodType,brand,date,costprice,sellprice FROM egg  ORDER BY eggid";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public ResultSet getEggSearch(String text) {
        try {
