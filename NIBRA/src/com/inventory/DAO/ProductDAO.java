...
public class ProductDAO {
    ...
    public String getBrand(String prodCode) {
        String Brand = null;
        try {
            String query = "SELECT brand FROM products WHERE productcode='" + prodCode + "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next()) {
    ...
    public void addFunction(ProductDTO productDTO) {
        try {
            ...
            prepStatement.setString(6, productDTO.getProdType());
            ...
            JOptionPane.showMessageDialog(null, "Produto adcionado e esta pronto para a venda.");
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    }
    ...
    public void deleteSaleDAO(int ID) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Transacao removida");
        } catch (SQLException e) {
            e.printStackTrace();
        }
        deleteStock();
    ...
    public ResultSet getStockFromCode(String text) {
        try {
            String query = "SELECT productcode,quantity,prodType  FROM currentstock "
                    + "WHERE productcode='" + text + "' LIMIT 1";
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        public int getQuantityFromCode(String text) {
        int quantity = 0;
        try {
            String query = "SELECT quantity FROM currentstock "
                    + "WHERE productcode='" + text + "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next()) {
                quantity = resultSet.getInt("quantity");
            }
